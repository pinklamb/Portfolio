...
import { renderCheckout } from './checkoutHeader.js';
...
export function renderOrderSummary() {
  ...
  cart.forEach((cartItem) => {
    ...
    const today = dayjs();
    const deliveryDate = today.add(deliveryOption.shippingDays,
      'days'
    );
    const displayDate = deliveryDate.format(
      'dddd, MMMM, D'
    );
  ...
  function deliveryOptionsHTML(matchingProduct, cartItem) {
    ...
    deliveryOptions.forEach((deliveryOption) => {
      ...
      const displayPrice = deliveryOption.priceCents === 0
        ? 'FREE'
